        ...
        public void Update()
        {
            if (photonView.IsMine)
            {
                ...
                if (players.Length == 2)
                {
                    ...
                    ObiRope[] ropes = obiSolver1.GetComponent<ObiSolver>().GetComponentsInChildren<ObiRope>();
                    ropes[0].GetComponent<MeshRenderer>().enabled = true;
                    ObiParticleAttachment[] attachments1 = ropes[0].GetComponents<ObiParticleAttachment>();
                    attachments1[1].target = players[0].transform;
                    ropes[1].GetComponent<MeshRenderer>().enabled = true;
                    ObiParticleAttachment[] attachments2 = ropes[1].GetComponents<ObiParticleAttachment>();
                    attachments2[1].target = players[1].transform;
